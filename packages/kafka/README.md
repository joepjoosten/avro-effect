...
import { registryValueDeserializer, registryValueSerializer } from "@avro-effect/kafka"

const registry = makeClient({ endpoint: "http://localhost:8081" })
...
const serialize = registryValueSerializer(registry, {
  topic: "events",
  schema
})

const deserialize = registryValueDeserializer(registry)
```
